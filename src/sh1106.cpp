...
void draw_string(int x, int y, const char *str)
{
    while (*str)
    {
        ...
        {
            // 2 octets — accents courants (0x80 à 0x7FF)
            code = ((c & 0x1F) << 6) | ((uint8_t)str[1] & 0x3F);
            str += 2;
        }
        else if ((c & 0xF0) == 0xE0)
        {
            // 3 octets — caractères rares (0x800 à 0xFFFF)
            code = ((c & 0x0F) << 12) | (((uint8_t)str[1] & 0x3F) << 6) | ((uint8_t)str[2] & 0x3F);
            str += 3;
        }
        else
        {
            // octet invalide, on saute
            str += 1;
            continue;
        }
